...
const orderLink = computed(() => {
  ...
  const params = new URLSearchParams({
    ...
    desc: 'Световой короб (лайтбокс)'
  })
  return `/order?${params.toString()}`
})

const cartItemDescription = computed(() => 'Световой короб (лайтбокс)')
...
const calculatorDataForCart = computed(() => ({
  params: [
    ...
    { variable: 'type', type: 5, value: calculatorData.type },
    { variable: 'bok', type: 5, value: calculatorData.bok },
    { variable: 'light', type: 5, value: calculatorData.light }
  ],
  mat_select_params: []
}))
...
const calculatePrice = async () => {
  try {
    ...
    const params = [
      { variable: 'w', type: 1, value: calculatorData.w },
      { variable: 'h', type: 1, value: calculatorData.h },
      { variable: 'type', type: 5, value: getIndex('type', calculatorData.type) },
      { variable: 'bok', type: 5, value: getIndex('bok', calculatorData.bok) },
      { variable: 'light', type: 5, value: getIndex('light', calculatorData.light) }
    ]
