 ...
 * Получает данные из калькулятора через GET параметры
 */

// Получаем параметры из GET
$calc_position_id = isset($_GET['calc_position_id']) ? intval($_GET['calc_position_id']) : 0;
$price_good = isset($_GET['price']) ? intval($_GET['price']) : 0;
$description = isset($_GET['desc']) ? htmlspecialchars($_GET['desc']) : 'Заказ';

// Проверка обязательных параметров
if (!$calc_position_id || !$price_good) {
    die('Ошибка: отсутствуют данные заказа. <a href="/">Вернуться на главную</a>');
}
?>
<!DOCTYPE html>
<html lang="ru">
...
<body>
			...
			<div class="header-inner">
				<div class="flex">
					<div class="logo-block">
						<div class="logo"><a href="#"><img src="../dev/public/img/dest/logo.svg" alt=""></a></div>
						<div class="city">
							<a href="#">
								<span>Самара</span>
					...
					<div class="grid">
						<div class="left-block">
							<div class="design-price">
								...
								<div class="all-price">
									...
									<div class="val" id="totalPrice"><?php echo number_format($price_good, 0, ',', ' '); ?> ₽</div>
								</div>
							</div>
						</div>
						<div class="right-block">
							...
							<form class="form-block" id="orderForm">
								...
								<input type="hidden" name="calc_position_id" value="<?php echo $calc_position_id; ?>">
								<input type="hidden" name="price_good" value="<?php echo $price_good; ?>">
	...
	<footer>
		<div class="container">
			<div class="footer-top">
				<div class="flex">
					<div class="logo-block">
						<div class="logo"><a href="#"><img src="../dev/public/img/dest/footer-logo.svg" alt=""></a></div>
						<div class="contacts">
	...
	document.getElementById('orderForm').addEventListener('submit', async function(e) {
		...
		try {
			...
			const { client_id } = await contactRes.json();
			...
			const { calc_id } = await calcRes.json();

			// 3. TODO: Если загружен файл - добавляем ссылку
			// const fileInput = document.getElementById('designFile');
			// if (fileInput.files.length > 0) {
			//     // Здесь нужно загрузить файл и получить ссылку
			//     // await fetch('/backend/api/calc/addLink', ...)
			// }

			// 4. Сохраняем калькуляцию
			const saveRes = await fetch('/backend/api/calc/saveCalc', {
				method: 'POST',
				headers: {'Content-Type': 'application/json'},
				body: JSON.stringify({ calc_id, client_id })
			});
